#!/usr/bin/env python3
"""AIOps break-even model for W3-D3."""


def is_worth_it(
    num_services: int,
    incidents_per_month: int,
    avg_incident_duration_hours: float,
    downtime_cost_per_hour: float,
    expected_mttr_reduction_pct: float = 0.4,
    aiops_monthly_cost: float = 15_000,
) -> dict:
    """
    Returns:
      {
        "monthly_value": float,
        "monthly_cost": float,
        "roi": float,
        "payback_months": float,  # or float('inf')
        "verdict": "worth_it" | "marginal" | "not_worth_it"
      }
    Verdict rule:
      roi > 1.5 -> worth_it
      1.0 < roi <= 1.5 -> marginal
      roi <= 1.0 -> not_worth_it
    """
    if num_services < 0:
        raise ValueError("num_services must be non-negative")
    if incidents_per_month < 0:
        raise ValueError("incidents_per_month must be non-negative")
    if avg_incident_duration_hours < 0:
        raise ValueError("avg_incident_duration_hours must be non-negative")
    if downtime_cost_per_hour < 0:
        raise ValueError("downtime_cost_per_hour must be non-negative")
    if expected_mttr_reduction_pct < 0:
        raise ValueError("expected_mttr_reduction_pct must be non-negative")
    if aiops_monthly_cost < 0:
        raise ValueError("aiops_monthly_cost must be non-negative")

    monthly_value = (
        incidents_per_month
        * avg_incident_duration_hours
        * downtime_cost_per_hour
        * expected_mttr_reduction_pct
    )
    monthly_cost = aiops_monthly_cost
    roi = monthly_value / monthly_cost if monthly_cost else float("inf")
    payback_months = monthly_cost / monthly_value if monthly_value > monthly_cost else float("inf")

    if roi > 1.5:
        verdict = "worth_it"
    elif roi > 1.0:
        verdict = "marginal"
    else:
        verdict = "not_worth_it"

    return {
        "monthly_value": float(monthly_value),
        "monthly_cost": float(monthly_cost),
        "roi": float(roi),
        "payback_months": float(payback_months),
        "verdict": verdict,
    }


if __name__ == "__main__":
    print(is_worth_it(num_services=20, incidents_per_month=2,
                      avg_incident_duration_hours=1, downtime_cost_per_hour=10_000,
                      aiops_monthly_cost=15_000))
    print(is_worth_it(num_services=100, incidents_per_month=5,
                      avg_incident_duration_hours=2, downtime_cost_per_hour=20_000,
                      aiops_monthly_cost=25_000))
    # B2B ecommerce checkout: $35k/hour is defensible because a major checkout
    # outage blocks order capture during business hours and creates support cost.
    print(is_worth_it(num_services=45, incidents_per_month=3,
                      avg_incident_duration_hours=1.5, downtime_cost_per_hour=35_000,
                      aiops_monthly_cost=18_000))
