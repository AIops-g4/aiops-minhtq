...
- Entry point chính là `phase3/techx-corp-platform/src/product-reviews/product_reviews_server.py`.
...
- Valkey hiện có dùng lại cho AI cache, pending action và conversation state bằng namespace riêng.
...
| --- | --- | --- | --- | --- |
| 1 | A1.1 Verified Summarization, Grounding, and Citations | P1 | None | Nền tảng để AI không trả lời thiếu bằng chứng. |
| 2 | A1.2 Prompt Injection, PII, and Tool Guardrails | P1 | None | Cần trước khi mở rộng tool surface và gửi thêm dữ liệu vào LLM. |
| 3 | A2.1 Natural Language Product Discovery | P1 | A1.2 | Entry point của Shopping Copilot; kết quả phải đến từ catalog thật. |
| 4 | A2.2 Review-Grounded Product Q&A | P1 | A1.1, A2.1 | Dùng grounding của A1.1 để trả lời câu hỏi về sản phẩm đã tìm được. |
| 5 | A2.3 Confirmation-Controlled Cart Actions | P1 | A1.2, A2.1 | Chỉ chuẩn bị cart action sau khi có product ID đáng tin và guardrail. |
| 6 | A1.3 Resilience and Cost Optimization | P2 | A1.1, A1.2 | Tối ưu timeout, fallback, cache, metric cho luồng đã được validate. |
| 7 | A2.4 Multi-Turn Conversation and Bounded Orchestration | P2 | A2.1, A2.2, A2.3 | Là lớp hội thoại nhiều lượt, nên làm sau khi các capability đơn lượt ổn định. |
...
- Dùng **DeepEval** cho offline eval grounding/hallucination; không đưa eval framework vào runtime path.
...
**What:** Cho phép người dùng mô tả nhu cầu như giá, category, feature. Hệ thống chuyển nhu cầu đó thành search/filter/rank trên catalog và trả kết quả có product ID thật.
...
- Tận dụng `ProductCatalogService.SearchProducts` hiện có.
- Tận dụng `product_catalog_stub` đã có trong `product-reviews`.
- Dùng **Instructor + Pydantic** nếu cần structured intent parsing.
- Chưa dùng semantic/embedding search trong MVP; chỉ cân nhắc sau nếu catalog search hiện tại không đủ.
...
**What:** Thêm conversation state ngắn hạn để lưu product references và pending action. Agent execution phải có giới hạn rõ về số vòng, số tool call và deadline.

**Acceptance Criteria:**

- Hệ thống resolve được reference trong cùng conversation.
- Conversation state được cách ly theo user/session.
...
- Tận dụng Valkey hiện có cho conversation state.
- Tận dụng tool calling flow hiện có, chỉ thêm giới hạn rõ về rounds/tool calls/deadline.
- Chưa dùng LangGraph/Temporal trong MVP; Python state machine là đủ cho bounded flow ban đầu.
